...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ app: UIApplication, open url: URL, options: [UIApplicationOpenURLOptionsKey : Any] = [:]) -> Bool {
        ...
        if (url.scheme == "twfollow") {
            if (url.host == "buycredits") {
                let mainStoryboard = UIStoryboard(name: "Main", bundle: Bundle.main)
                
                let rootVC: UINavigationController = self.window?.rootViewController as! UINavigationController
                let mainVC = mainStoryboard.instantiateViewController(withIdentifier: "MainViewController") as! MainViewController
                
                mainVC.openUrlId = "buycredits"
